...
export class HttpClient {
  ...
  private static loginPath = withBasePath('/login')
  private static rootPath = withBasePath('/')
  ...
  private async handleError(error: AxiosError): Promise<any> {
    ...
    if (status === 401) {
      ...
      if (!window.location.pathname.includes(HttpClient.loginPath)) {
        const returnUrl = window.location.pathname + window.location.search
        if (
          returnUrl !== HttpClient.loginPath &&
          returnUrl !== HttpClient.rootPath
        ) {
          sessionStorage.setItem('returnUrl', returnUrl)
        }

        sessionStorage.setItem('from401', 'true')
        window.location.href = HttpClient.loginPath

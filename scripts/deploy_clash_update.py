...
def main():
    ...
    steps = [
        ...
        ("安装 Python 依赖", f"cd {VPS_PATH} && pip3 install -r requirements.txt"),
        ("安装前端依赖", f"cd {VPS_PATH}/web && npm install"),
        ("构建前端", f"cd {VPS_PATH}/web && npm run build"),
        ("创建数据目录", f"mkdir -p {VPS_PATH}/data"),
        ("检查 Clash 配置", f"ls -la {VPS_PATH}/*.yaml"),
        ("重启 API 服务", "systemctl restart valuescan-api"),
        ("检查 API 服务状态", "systemctl status valuescan-api --no-pager -l"),
        ("重启信号监控", "systemctl restart valuescan-signal"),
        ("检查信号监控状态", "systemctl status valuescan-signal --no-pager -l"),
    ]

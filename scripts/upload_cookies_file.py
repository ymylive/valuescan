#!/usr/bin/env python3
"""
Upload a local ValueScan cookies file to a VPS and restart services.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upload ValueScan cookies file to VPS.")
    parser.add_argument(
        "--file",
        required=True,
        help="Path to local cookies json file",
    )
    ...
    parser.add_argument(
        "--services",
        default=os.getenv("VALUESCAN_COOKIE_SERVICES", "valuescan-monitor valuescan-api"),
    ...
    if not local_path.exists():
        print(f"Cookies file not found: {local_path}")
        return 1
    ...
    try:
        ...
        remote_file = f"{remote_dir}/valuescan_cookies.json"

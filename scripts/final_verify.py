...
# 4. NOFX 页面访问
print("\n4. NOFX 页面访问 (HTTPS):")
stdin, stdout, stderr = ssh.exec_command('curl -sk --max-time 5 -o /dev/null -w "%{http_code}" https://cornna.qzz.io/nofx/')
print(f"  HTTP 状态码: {stdout.read().decode()}")

# 5. ValueScan 前端
print("\n5. ValueScan 前端 (HTTPS):")
stdin, stdout, stderr = ssh.exec_command('curl -sk --max-time 5 -o /dev/null -w "%{http_code}" https://cornna.qzz.io/')
print(f"  HTTP 状态码: {stdout.read().decode()}")
...
访问地址:
  - ValueScan: https://cornna.qzz.io/
  - NOFX: https://cornna.qzz.io/nofx/

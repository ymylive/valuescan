...
time.sleep(1)
ssh.exec_command("systemctl restart valuescan-signal valuescan-monitor valuescan-watchdog valuescan-token-refresher")
time.sleep(5)

print("\n📊 服务状态:")
print("="*80)
stdin, stdout, stderr = ssh.exec_command("systemctl status valuescan-signal valuescan-monitor valuescan-watchdog --no-pager -l")
print(stdout.read().decode())

print("\n📋 valuescan-monitor 日志 (最近200行):")
print("="*80)
stdin, stdout, stderr = ssh.exec_command("journalctl -u valuescan-monitor -n 200 --no-pager")
print(stdout.read().decode())

...
ASYNC_FRONTEND = os.environ.get("VALUESCAN_VPS_ASYNC_FRONTEND", "").strip() == "1"
SKIP_CLEANUP = os.environ.get("VALUESCAN_VPS_SKIP_CLEANUP", "").strip() == "1"
...
def main():
    ...
    steps = []
    steps.append(("停止旧服务", "systemctl stop valuescan-signal 2>/dev/null || true"))
    steps.append(("禁用旧服务", "systemctl disable valuescan-signal 2>/dev/null || true"))
    ...
    cleanup_cmd = (
        "rm -rf "
        f"{VPS_PATH}/binance_trader "
        f"{VPS_PATH}/trader "
        f"{VPS_PATH}/telegram_copytrade "
        f"{VPS_PATH}/backtest "
        f"{VPS_PATH}/simulation "
        f"{VPS_PATH}/manager "
        f"{VPS_PATH}/decision "
        f"{VPS_PATH}/debate "
        f"{VPS_PATH}/experience "
        f"{VPS_PATH}/store/strategy.go "
        f"{VPS_PATH}/store/position.go "
        f"{VPS_PATH}/docs/community "
        f"{VPS_PATH}/docs/getting-started "
        f"{VPS_PATH}/docs/guides "
        f"{VPS_PATH}/docs/i18n "
        f"{VPS_PATH}/web/src/components/Config/CopyTradeConfig.tsx "
        f"{VPS_PATH}/web/src/components/Config/RiskManagementConfig.tsx "
        f"{VPS_PATH}/web/src/components/Config/TradingBotConfig.tsx "
        f"{VPS_PATH}/web/src/components/Trading "
        f"{VPS_PATH}/web/src/pages/Exchanges.tsx "
        f"{VPS_PATH}/web/src/pages/Models.tsx "
        f"{VPS_PATH}/web/src/pages/PerformanceStats.tsx "
        f"{VPS_PATH}/web/src/pages/PositionMonitor.tsx "
        f"{VPS_PATH}/web/src/pages/Settings.tsx "
        f"{VPS_PATH}/web/src/pages/Strategies.tsx "
        f"{VPS_PATH}/web/src/pages/TraderDetails.tsx "
        f"{VPS_PATH}/web/src/pages/Traders.tsx "
        f"{VPS_PATH}/web/src/pages/TradingHistory.tsx "
        f"{VPS_PATH}/web/src/services/tradingApi.ts "
        f"{VPS_PATH}/web/src/types/trading.ts "
        "&& rm -f /etc/systemd/system/valuescan-trader.service "
    ...
    if ASYNC_FRONTEND:
        frontend_cmd = (
            ...
            "mkdir -p /var/www/valuescan && cp -r dist/* /var/www/valuescan/ && "

...
def deploy_valuescan(ssh: paramiko.SSHClient, sftp: paramiko.SFTPClient) -> None:
    ...
    file_uploads = [
        (Path("signal_monitor/ai_signal_analysis.py"), f"{VALUESCAN_DIR}/signal_monitor/ai_signal_analysis.py"),
        (Path("signal_monitor/ai_market_summary.py"), f"{VALUESCAN_DIR}/signal_monitor/ai_market_summary.py"),
        (Path("signal_monitor/telegram.py"), f"{VALUESCAN_DIR}/signal_monitor/telegram.py"),
        (Path("web/src/pages/Dashboard.tsx"), f"{VALUESCAN_DIR}/web/src/pages/Dashboard.tsx"),
        (Path("web/src/locales/zh.json"), f"{VALUESCAN_DIR}/web/src/locales/zh.json"),
    ]
    ...
    print("\nBuilding ValueScan frontend on VPS...")
    result = _exec(
        ssh,
        f"cd {VALUESCAN_DIR}/web && npm install && npm run build",
        timeout=900,
    )
...
""",
    ...
    print(result)
    print("\nRestarting nginx...")
    result = _exec(ssh, "systemctl restart nginx")
    print(result if result else "  [OK]")
...
def restart_services(ssh: paramiko.SSHClient) -> None:
    ...
    print("\nRestarting valuescan-monitor...")
    result = _exec(ssh, "systemctl restart valuescan-monitor")
    print(result if result else "  [OK]")

    print("\nDisabling legacy valuescan-signal...")
    result = _exec(ssh, "systemctl disable --now valuescan-signal || true")
    print(result if result else "  [OK]")

    print("\nChecking service status...")
    result = _exec(ssh, "systemctl status valuescan-api valuescan-monitor valuescan-signal --no-pager | head -30")

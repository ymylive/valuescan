...
const SERVICE_INFO: Record<ServiceName, { icon: React.ElementType; color: string }> = {
  signal: { icon: Zap, color: 'text-yellow-400' },
  trader: { icon: Bot, color: 'text-blue-400' },
  copytrade: { icon: Eye, color: 'text-green-400' },
  keepalive: { icon: Shield, color: 'text-purple-400' },
}

const SERVICE_NAMES: Record<ServiceName, { zh: string; en: string }> = {
  signal: { zh: '信号监控', en: 'Signal Monitor' },
  trader: { zh: '自动交易', en: 'Auto Trader' },
  copytrade: { zh: '跟单系统', en: 'Copy Trade' },
  keepalive: { zh: '服务监控', en: 'Service Monitor' },
}

const SERVICE_DESCS: Record<ServiceName, { zh: string; en: string }> = {
  signal: { zh: 'ValueScan 信号捕获', en: 'ValueScan signal capture' },
  trader: { zh: 'Binance 合约交易', en: 'Binance futures trading' },
  copytrade: { zh: 'Telegram 信号跟单', en: 'Telegram signal copy' },
  keepalive: { zh: '自动重启守护', en: 'Auto restart daemon' },
}
...
export default function App() {
  ...
  const handleServiceAction = async (
    ...
    try {
      ...
      if (result.success) {
        const actionNames = {
          start: language === 'zh' ? '已启动' : 'started',
          stop: language === 'zh' ? '已停止' : 'stopped',
          restart: language === 'zh' ? '已重启' : 'restarted',
        }
        toast.success(`${SERVICE_NAMES[service][language]} ${actionNames[action]}`)
        loadStatus()
      } else {
        toast.error(t('saveFailed'))
      }
    } catch (e) {
      toast.error(t('saveFailed') + ': ' + e)
    } finally {
  ...
  const loadConfig = async () => {
    ...
    try {
      const [
        ...
        api.getAISignalConfig().catch(() => null),
        api.getAILevelsConfig().catch(() => null),
        api.getAIOverlaysConfig().catch(() => null),
        api.getAIMarketConfig().catch(() => null),
      ])

      const mergedSignal = { ...(configData?.signal || {}) }
      if (aiSignalData) {
        mergedSignal.ai_signal_config = aiSignalData
        if (typeof aiSignalData.enabled === 'boolean') {
          mergedSignal.enable_ai_signal_analysis = aiSignalData.enabled
        }
      }
      if (aiLevelsData) {
        mergedSignal.ai_levels_config = aiLevelsData
        if (typeof aiLevelsData.enabled === 'boolean') {
          mergedSignal.enable_ai_key_levels = aiLevelsData.enabled
        }
      }
      if (aiOverlaysData) {
        mergedSignal.ai_overlays_config = aiOverlaysData
        if (typeof aiOverlaysData.enabled === 'boolean') {
          mergedSignal.enable_ai_overlays = aiOverlaysData.enabled
        }
      }
      if (aiMarketData) {
        mergedSignal.ai_market_config = aiMarketData
      }
    ...
    try {
      if (activeConfigSection !== 'keepalive') {
        ...
        if (result.success) {
          ...
          if (result.restarted) {
            toast.info(t('serviceRestarting'))
          }
          setUnsavedChanges(false)
        } else {
          toast.error(result.errors?.join(', ') || t('saveFailed'))
        }
        
        // Save AI configs separately
        const signalConfig = config?.signal
        if (signalConfig) {
          try {
            if (signalConfig.ai_signal_config) {
              await api.saveAISignalConfig(signalConfig.ai_signal_config)
            }
            if (signalConfig.ai_levels_config) {
              await api.saveAILevelsConfig(signalConfig.ai_levels_config)
            }
            if (signalConfig.ai_overlays_config) {
              await api.saveAIOverlaysConfig(signalConfig.ai_overlays_config)
            }
            if (signalConfig.ai_market_config) {
              await api.saveAIMarketConfig(signalConfig.ai_market_config)
            }
          } catch (aiErr) {
            console.error('Failed to save AI configs:', aiErr)
          }
        }
      }
      if (activeConfigSection === 'keepalive' && keepaliveConfig) {
        ...
        if (result.success) {
          toast.success('Keepalive ' + t('configSaved'))
          setUnsavedChanges(false)
        } else {
          toast.error(result.errors?.join(', ') || result.error || t('saveFailed'))
        }
      }
    } catch (e) {
      toast.error(t('saveFailed') + ': ' + e)
                    ...
                    <ScaleIn key={key} delay={index * 0.1} className="h-full">
                      ...
                      >
                        ...
                        <h3 className="text-lg font-semibold mb-1">
                          {SERVICE_NAMES[key][language]}
                        </h3>
                        <p className="text-xs text-neutral-500 mb-6">
                          {SERVICE_DESCS[key][language]}
                        </p>
                ...
                <>
                  ...
                  <SlideUp delay={0.1}>
                    <div className="flex gap-2 p-1 bg-white/5 rounded-xl backdrop-blur-md border border-white/5 overflow-x-auto">
                      {[
                        { id: 'signal' as const, label: t('signal'), icon: Zap },
                        { id: 'trader' as const, label: t('trader'), icon: Bot },
                        {
            ...
            >
              <SlideUp className="flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    <option value="signal">{t('signalMonitor')}</option>
                    <option value="trader">{t('autoTrader')}</option>

...
export const api = {
  ...
  async getAISignalConfig(): Promise<any> {
    const result = await httpClient.get<any>(`${API_BASE}/valuescan/ai-signal/config`)
    if (!result.success) throw new Error('获取 AI 简评配置失败')
    return result.data?.config || {}
  },

  async saveAISignalConfig(config: any): Promise<any> {
    const result = await httpClient.post<any>(`${API_BASE}/valuescan/ai-signal/config`, { config })
    if (!result.success) throw new Error('保存 AI 简评配置失败')
    return result.data
  },

  async testAISignalConnection(config: any): Promise<any> {
    const result = await httpClient.post<any>(`${API_BASE}/valuescan/ai-signal/test`, config)
    return result.data || result
  },

  // AI Key Levels Config (AI主力位配置)
  async getAILevelsConfig(): Promise<any> {
    const result = await httpClient.get<any>(`${API_BASE}/valuescan/ai-levels/config`)
    if (!result.success) throw new Error('获取 AI 主力位配置失败')
    return result.data?.config || {}
  },

  async saveAILevelsConfig(config: any): Promise<any> {
    const result = await httpClient.post<any>(`${API_BASE}/valuescan/ai-levels/config`, { config })
    if (!result.success) throw new Error('保存 AI 主力位配置失败')
    return result.data
  },

  async testAILevelsConnection(config: any): Promise<any> {
    const result = await httpClient.post<any>(`${API_BASE}/valuescan/ai-levels/test`, config)
    return result.data || result
  },

  // AI Overlays Config (AI辅助线/叠加层配置)
  async getAIOverlaysConfig(): Promise<any> {
    const result = await httpClient.get<any>(`${API_BASE}/valuescan/ai-overlays/config`)
    if (!result.success) throw new Error('获取 AI Overlays 配置失败')
    return result.data?.config || {}
  },

  async saveAIOverlaysConfig(config: any): Promise<any> {
    const result = await httpClient.post<any>(`${API_BASE}/valuescan/ai-overlays/config`, { config })
    if (!result.success) throw new Error('保存 AI Overlays 配置失败')
    return result.data
  },

  async testAIOverlaysConnection(config: any): Promise<any> {
    const result = await httpClient.post<any>(`${API_BASE}/valuescan/ai-overlays/test`, config)
    return result.data || result
  },

  // AI Market Analysis Config (AI市场宏观分析配置)
  async getAIMarketConfig(): Promise<any> {
    const result = await httpClient.get<any>(`${API_BASE}/valuescan/ai-market/config`)
    if (!result.success) throw new Error('获取 AI 市场分析配置失败')
    return result.data?.config || {}
  },

  async saveAIMarketConfig(config: any): Promise<any> {
    const result = await httpClient.post<any>(`${API_BASE}/valuescan/ai-market/config`, { config })
    if (!result.success) throw new Error('保存 AI 市场分析配置失败')
    return result.data
  },

  async testAIMarketConnection(config: any): Promise<any> {
    const result = await httpClient.post<any>(`${API_BASE}/valuescan/ai-market/test`, config)
    return result.data || result
  },
  ...
  async getValuescanEnv(): Promise<Record<string, string>> {
    const result = await httpClient.get<{ env: Record<string, string> }>(
      `${API_BASE}/valuescan/env`
    )
    if (!result.success) {
  ...
  async saveValuescanEnv(
    ...
    const result = await httpClient.post<{
      ...
      message?: string
      error?: string
    }>(`${API_BASE}/valuescan/env`, { env })
    return result.success
      ? result.data!
      : { success: false, error: result.message || '保存失败' }
  },

...
export class ConfigService {
  ...
  async saveConfiguration(config: CompleteConfig): Promise<void> {
    ...
    try {
      ...
      // Don't throw error, as we've saved to local storage
    }
  }
  ...
  private transformBackendToFrontend(data: BackendConfig): CompleteConfig {
    ...
    const signal_monitor: SignalMonitorConfig = {
      ...
      auto_relogin: signal.auto_relogin || false,
      auto_relogin_cooldown: signal.auto_relogin_cooldown || 1800,
      ...
      token_refresh_interval_hours: signal.token_refresh_interval_hours || 0.8,
      token_refresh_safety_seconds: signal.token_refresh_safety_seconds || 300,
      login_method: signal.login_method || 'auto',
      refresh_window_start: signal.refresh_window_start || 0,
      refresh_window_end: signal.refresh_window_end || 6,

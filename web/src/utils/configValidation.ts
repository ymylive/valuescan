...
export function validateNumericRange(
  ...
  if (min !== undefined && value < min) {
    return { valid: false, message: `Value must be >= ${min}` }
  }

  if (max !== undefined && value > max) {
    return { valid: false, message: `Value must be <= ${max}` }
  }
...
export function validateField(
  ...
  if (schema?.pattern && typeof value === 'string') {
    ...
    if (!regex.test(value)) {
      return { field: key, message: 'Invalid format', type: 'format' }
    }
  }
...
export function validateKeepaliveCheckInterval(value: number): {
  ...
  if (typeof value !== 'number' || isNaN(value)) {
    return {
      valid: false,
      clamped: 60,
      message: 'Check interval must be a valid number',
    }
  }
  ...
  return {
    valid,
    clamped,
    message: valid ? undefined : `Check interval must be ${MIN}-${MAX} seconds`,
  }
}
  ...
  if (config.enabled) {
    if (!config.bot_token?.trim()) {
      errors.push({
        ...
        message: 'Bot Token is required when Telegram alerts are enabled',
        type: 'required',
      })
    }

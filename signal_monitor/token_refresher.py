...
def is_token_valid(buffer_seconds: int = TOKEN_REFRESH_SAFETY_SECONDS) -> bool:
    """
    Return True when a non-empty account_token exists and is not expiring soon.
    """
    data = _load_localstorage()
    token = (data.get("account_token") or "").strip()
    if not token:
...
def refresh_if_needed(
    ...
    needs_login = force or (not account_token) or (seconds_left is None) or (seconds_left <= TOKEN_REFRESH_SAFETY_SECONDS)
...
def run_refresh_loop(email: str, password: str, interval_hours: float = TOKEN_REFRESH_INTERVAL_HOURS) -> None:
    ...
    while True:
        try:
            refreshed = refresh_if_needed(email, password, headless=True)
            expiry = get_token_expiry()
            if refreshed and expiry:
                logger.info("Token valid until %s", expiry.isoformat())
                # Sleep until safety window or provided interval, whichever is smaller
                seconds_left = _seconds_until_expiry(_load_localstorage().get("account_token", ""))
                sleep_seconds = interval_hours * 3600
                if seconds_left is not None:
                    sleep_seconds = max(30, min(sleep_seconds, max(0, seconds_left - TOKEN_REFRESH_SAFETY_SECONDS)))
            else:

...
try:
    ...
    AUTO_RELOGIN = (os.getenv("VALUESCAN_AUTO_RELOGIN") == "1") if os.getenv("VALUESCAN_AUTO_RELOGIN") else getattr(signal_config, "AUTO_RELOGIN", False)
    AUTO_RELOGIN_COOLDOWN = int(os.getenv("VALUESCAN_AUTO_RELOGIN_COOLDOWN") or getattr(signal_config, "AUTO_RELOGIN_COOLDOWN", 1800))
except ImportError:
...
DISABLE_HTTP_REFRESH = os.getenv("VALUESCAN_DISABLE_HTTP_REFRESH", "0") in ("1", "true", "True")
CHROME_REFRESH_COOLDOWN = int(os.getenv("VALUESCAN_CHROME_REFRESH_COOLDOWN", "300"))
CHROME_REFRESH_TIMEOUT = int(os.getenv("VALUESCAN_CHROME_REFRESH_TIMEOUT", "240"))
...
def _trigger_chrome_refresh(reason: str) -> bool:
    global _LAST_CHROME_REFRESH_MONO

    now = time.monotonic()
    if now - _LAST_CHROME_REFRESH_MONO < CHROME_REFRESH_COOLDOWN:
        logger.info("Chrome refresh skipped (cooldown).")
        return False

    base_dir = Path(__file__).resolve().parent
    cdp_script = base_dir / "cdp_token_refresher.py"
    ui_script = base_dir / "token_refresher.py"

    env = dict(os.environ)
    env.setdefault("VALUESCAN_TOKEN_FILE", str(TOKEN_FILE))

    logger.warning("Token unavailable (%s); starting Chromium refresh.", reason)
    ...
    def _run(script: Path) -> bool:
        ...
        try:
            res = subprocess.run(
                [sys.executable or "python3", str(script), "--once"],
                cwd=str(script.parent),
                env=env,
                capture_output=True,
                text=True,
                timeout=CHROME_REFRESH_TIMEOUT,
            )
        except Exception as exc:
            logger.warning("Chromium refresh failed to start: %s", exc)
            return False
        if res.returncode == 0:
            data = _load_localstorage(retries=3)
            if (data.get("account_token") or "").strip():
        ...
        if msg:
            logger.warning("Chromium refresh output: %s", msg[-300:])
        return False
    ...
    _LAST_CHROME_REFRESH_MONO = time.monotonic()
    if ok:
        logger.info("Chromium refresh succeeded.")
    return ok
    ...
    while True:
        try:
            ...
            if not account_token:
                if _trigger_chrome_refresh("account_token_missing"):
                    consecutive_failures = 0
                    time.sleep(1)
                    continue

                logger.warning("account_token 缺失，等待 Chrome token 刷新服务...")
            ...
            if status == "ok" and payload and process_response_data:
                ...
                logger.warning("Token 过期，等待 Chrome token 刷新服务自动刷新...")
                consecutive_failures += 1
                time.sleep(30)
